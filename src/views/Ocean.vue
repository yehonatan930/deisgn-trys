<template>
  <div id="ocean">
    <div class="logo-circle">
      <font-awesome-icon icon="ship" class="ship-logo" />
    </div>
    <div class="landscape">
      ...
      <div class="trash-container">
        <div v-for="icon in trashIcons" :key="icon">
          <font-awesome-icon
            class="trash"
            v-for="index in randomNumberBetween(1, 10)"
            :key="index"
            :icon="icon"
            :style="getTrashStyle()"
          />
    ...
    <h1 class="money">{{ currencyFormatter.format(this.moneyRaised) }}</h1>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      trashFormatter: new Intl.NumberFormat("en-us", {
  ...
  computed: {
    moneyRaised() {
      return this.randomNumberBetween(0, this.maxMoney);
    },
    amountLeftToRaiseText() {
      return this.trashFormatter.format(
        Math.max(this.maxMoney - this.moneyRaised, 0)
      );
    },
    trashIcons() {
      return [
  ...
  methods: {
    ...
    getTrashStyle() {
      const top = this.randomNumberBetween(0, 50);
      const size = top / 5 + 1;
      return {
        top: `${top}vh`,
        width: `${size}vmin`,
        height: `${size}vmin`,
        left: `${this.randomNumberBetween(0, 100)}vw`,
        "--rotation": `${this.randomNumberBetween(-20, 20)}deg`,
      };
    },
  },
};
...
@keyframes sway {
  ...
  vfrom {
    transform: rotate(20deg) translateY(1vh);
  }

  to {
    transform: rotate(calc(20deg + 10deg)) translateY(0);
  }
}
